...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser k p = P (\i -> case (parse p i) of
                              ErrorResult e -> ErrorResult e
                              Result bi a -> parse (k a) bi)
-- k (a -> Parser b)
-- p Parser a
-- parse p :: Input -> ParseResult a
-- i :: Input
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) a1 a2 = P (\i -> case parse a1 i of
                        ErrorResult _ -> parse a2 i
--                        Result j a -> Result j a
--                        r -> r                    -- less safe "overlapping pattern match"
--                        r@(Result _ _) -> r
                        r@Result {} -> r
                        )
...
-- parser that runs 1 or many p OR always Nil
...
list1 ::
  ...
  -> Parser (List a)
-- 1 or many p
--list1 p = flbindParser p (\x -> 
--            flbindParser (list p) (\y ->
--            valueParser (x :. y)))
--list1 p = p >>= (\x -> 
--          (list p) >>= (\y ->
--            valueParser (x :. y)))
list1 p = 
-- monad comprehension, "do notation" in Haskell, "LINKQ" in C#
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
--satisfy f = character >>= \c -> case f c of
--                                    False -> failed
--                                    True -> pure c
satisfy f = character >>= 
--        lift3 bool unexpectedCharParser pure f
        bool <$> unexpectedCharParser <*> pure <*> f
-- character and then [call it c]
-- if p c then always c else fail/bool fail (alwaysc) (p c)
...
natural ::
  Parser Int
natural = bindParser(\k -> case read k of Empty -> failed
...
--    (list lower)
    ...
    <*> list lower
-- upper and then [call it u]
-- exactly 5 lower and then [call i l1]
-- (0 or more) lower and then [call it l2]
-- always (u :. l1 ++ l2)
...
-- phoneBodyParser and then [call i v]
-- is hash then [call it w]
-- always (u :. v ++ w)
...
personParser =
    ...
    <* spaces1
    <*> phoneParser
--  natural and then [call it a]
--  firstName and then  [call it b]
--  surname and then [call it c]
--  is smoker and then [call it d]
--  phone number and then [call it e]
--  always Person a b c d e
personParser2 =
    do 
        a <- ageParser
        spaces1
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) f a = flbindParser f (`mapParser` a)
